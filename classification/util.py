...
class Counter(dict):
    ...
    def argMax(self):
        ...
        if len(self) == 0:
            return None
        all_items = list(self.items())  # Convertimos a lista
    ...
    def __mul__(self, y):
        """
        Multiplying two counters gives the dot product.
        """
        sum = 0
        x = self
        if len(x) > len(y):
    ...
    def __sub__(self, y):
        """
        Subtracts another counter from this one.
        """
        addend = Counter()
        for key in self:
            addend[key] = self[key] - y.get(key, 0)
...
def normalize(vectorOrCounter):
    """
    Normalizes a vector or counter by dividing each value by the sum.
    """
...
def flipCoin(p):
    """
    Simulates a coin flip with probability p.
    """
...
def matrixAsList(matrix, value=True):
    ...
    """
    return [(row, col) for row in range(len(matrix)) for col in range(len(matrix[0])) if matrix[row][col] == value]
