...
def readlines(filename):
    ...
    if os.path.exists(filename):
        ...
        with zipfile.ZipFile('data.zip') as z:
            # Decodificamos los bytes a string antes de hacer split
...
# def loadPacmanStatesFile(filename, n):
#     with open(filename, 'rb') as f:  # Abre el archivo en modo binario
#         result = pickle.load(f)
#     return result
...
#     """
#     Helper function for file reading.
#     Converts elements to integers recursively.
#     """
#     if not isinstance(data, list):  # Verifica si data no es una lista
#         return int(data)  # Asegúrate de que se convierte a entero correctamente
#     else:
#         return [convertToInteger(item) for item in data]  # Devuelve una lista en lugar de un objeto map
